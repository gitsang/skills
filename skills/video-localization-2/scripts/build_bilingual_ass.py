...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="根据源语言 SRT 与逐段目标语言稿生成双语 ASS 字幕。"
    )
    ...
    parser.add_argument("--target-size", type=int, default=40, help="目标语言字幕字号")
    parser.add_argument("--source-size", type=int, default=28, help="源语言字幕字号")
...
def main() -> None:
    ...
    if len(srt_entries) != len(target_lines):
        raise SystemExit(
            f"Segment count mismatch: {len(srt_entries)} subtitle blocks vs {len(target_lines)} target-language lines"
        )
