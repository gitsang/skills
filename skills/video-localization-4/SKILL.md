...
Use ISO 639-1 two-letter codes throughout (e.g., `en`, `zh`, `ja`, `ko`, `es`, `fr`, `de`).

| Placeholder | Example | Description |
|---|---|---|
| `{src_lang}` | `en`, `zh`, `ja` | Source video language |
| `{tgt_lang}` | `zh`, `en`, `de` | Target narration/subtitle language |
| `{lang_pair}` | `en-zh`, `ja-en` | Source-to-target direction |
...
/path/to/video-loc/
  ...
  models/          # shared model weights
    ...
    Qwen3-TTS-12Hz-0.6B-Base -> Qwen3-TTS-12Hz-0___6B-Base  # symlink for repo ID compatibility
...
- Different target languages vary greatly in length; prefer concise rewriting over literal translation to avoid alignment issues.
...
- Copy or export all final files to `output/` with user-requested names (include language tags).
...
Replace `{src_lang}`, `{tgt_lang}`, `{lang_pair}` with actual language codes.
...
python scripts/transcribe_with_faster_whisper.py \
  ...
  # Add --language {src_lang} to force; omit for auto-detect

# Step 4: Generate translation prompt scaffold
...
# Step 6a: Full-script TTS (standard dubbing)
...
# Step 6b: Full-script TTS (voice cloning)
...
# Step 7: Per-segment aligned dubbing (standard)
...
# Step 7: Per-segment aligned dubbing (voice cloning)
python scripts/build_aligned_dub.py \
  --srt artifacts/transcript.{src_lang}.srt \
  --tgt-segments artifacts/script.{tgt_lang}.segments.txt \
  --video source/video.mp4 \
...
# Step 8: Bilingual subtitles
python scripts/build_bilingual_ass.py \
  --srt artifacts/transcript.{src_lang}.srt \
  --tgt-segments artifacts/script.{tgt_lang}.segments.txt \
  --ass-out artifacts/subtitles.{lang_pair}.ass
```

**Tesla P4 inference note**: Tesla P4 (compute capability 6.1) does not support flash-attn; each TTS segment takes ~8-10 s. 187 segments (~8 min video) totals ~25-30 minutes. Scripts support per-segment caching and resume after interruption.
...
Record failing domains and proxy variables. Do not misattribute network issues as script bugs. After proxy recovers, re-run original commands; if only a specific domain is unreachable, prefer mirror sources or local cache over changing the entire workflow.
...
- No `{src_lang}`, `{tgt_lang}`, and delivery language direction confirmed.
...
- [ ] Source language, target language, subtitle language combination, and delivery format confirmed.
