...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Per-segment timing report output path (JSON)",
    )
    parser.add_argument(
        "--segment-dir",
        type=Path,
        required=True,
        help="Per-segment audio cache directory (enables resume after interruption)",
    )
    parser.add_argument(
        "--backend",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "edge-tts voice name. Required when --backend=edge-tts. "
            "Run 'edge-tts --list-voices' and filter by target language code "
            "(e.g. zh-CN, fr-FR, de-DE, ja-JP) to find the right voice."
        ),
    )
    parser.add_argument(
        "--rate", default="+0%", help="edge-tts speech rate adjustment (e.g. +8%%)"
    )
    # qwen3-tts options
    parser.add_argument(
        "--reference-audio",
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Required when --backend=qwen3-tts (unless --x-vector-only): "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Use Qwen3-TTS x-vector-only mode: voice embedding only, "
            "no reference text prompt residue"
        ),
    )
    parser.add_argument(
        "--language",
        default=None,
        help=(
            "Target language hint for Qwen3-TTS (e.g. Chinese, French, Japanese). "
...
def read_segments(srt_path: Path, tgt_segments_path: Path) -> list[SubtitleSegment]:
    ...
    if len(tgt_lines) != len(src_segments):
        raise SystemExit(
            f"Segment count mismatch: {len(src_segments)} srt blocks vs "
            f"{len(tgt_lines)} target lines"
        )
...
def synthesize_segment(
    *,
    ...
    if backend == "edge-tts":
        ...
        asyncio.run(synthesize_edge_tts(text, output_path, voice, rate))
        return
    ...
    if reference_text is None and not x_vector_only:
        raise SystemExit(
            "--reference-text is required when --backend=qwen3-tts "
            "unless --x-vector-only is set"
        )
