...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Transcribe audio to source-language text and SRT using faster-whisper."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Source-language plain text output path",
    )
    parser.add_argument(
        "--srt-out", type=Path, required=True, help="Source-language SRT output path"
    )
    ...
    parser.add_argument(
        "--source-lang",
        default=None,
        help="Source language code (e.g., en, ja, ko, es). Auto-detected if omitted.",
    )
    parser.add_argument(
        "--disable-vad-filter",
        action="store_true",
        help="Disable vad_filter (enabled by default)",
    )
...
def main() -> None:
    ...
    if args.source_lang:
        kwargs["language"] = args.source_lang
