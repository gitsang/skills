...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate bilingual ASS subtitles from source SRT and translated segments."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--source-lang", default="en", help="Source language code (e.g., en, ja, ko)"
    )
    parser.add_argument(
        "--target-lang",
        default="zh",
        help="Target language code (e.g., zh, en, ja, ko, es, fr)",
    )
    parser.add_argument("--font-name", default="Noto Sans CJK SC", help="Font name")
    parser.add_argument("--font-size", type=int, default=36, help="Base font size")
    parser.add_argument(
        "--target-size", type=int, default=40, help="Target-language subtitle font size"
    )
    parser.add_argument(
        "--source-size", type=int, default=28, help="Source-language subtitle font size"
    )
...
def main() -> None:
    ...
    target_lines = [
        line.rstrip("\n")
        for line in args.translated_segments.read_text(encoding="utf-8").splitlines()
    ]
    if len(srt_entries) != len(target_lines):
        raise SystemExit(
            f"Segment count mismatch: {len(srt_entries)} subtitle blocks vs {len(target_lines)} translated lines"
        )
    ...
    for (start_raw, end_raw, source_text), target_text in zip(
        ...
        text = f"{{\\fs{args.target_size}}}{target_escaped}\\N{{\\fs{args.source_size}}}{source_escaped}"
